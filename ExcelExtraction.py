...
def create_sliced_df(all_dfs, start_row, end_row, date_dict=False):
	...
	for key, df in all_dfs.iteritems():
...
def verify_value_counts( Harmonised_DF):
		...
		for col in cols:
		    frequency_df.loc[len(frequency_df)]  =[ col, Harmonised_DF[col].count(), (Harmonised_DF[col].count()*100)/len(Harmonised_DF), set([ str(type(item)) for item in Harmonised_DF[col] ]) ]
